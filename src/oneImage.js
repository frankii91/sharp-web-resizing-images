...
router.get('/', async (req, res)=>
{
    try {
        ...
        const resultTyp = req.query.resultTyp || 'stream';
        const allowedResultTyp = {
            'local': true,
            'mount': true,
            'stream': true
        };
        if (!allowedResultTyp.hasOwnProperty(resultTyp)) return res.status(400).send('Invalid "resultTyp" parameter');
        const now = new Date();
        switch (resultTyp) {
            case "local":
                console.log(`resultTyp ${resultTyp} ... `);
